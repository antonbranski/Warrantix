...
//    private int selectedMarketIndex = -1;
    ...
    private static final int PERMISSIONS_REQUEST_READ_CONTACTS = 1182;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mIntent != null && mIntent.hasExtra(Constant.brandUpdatedFromSetting)) {
            marketFragment.getBrandDataAndSetAdapter();
        }
...
//        }
        ...
        requestMarshMallowPermission(0);
    }
...
//        if (!marshMallowPermission.checkPermissionForFineLocation() || !marshMallowPermission.checkPermissionForCoarseLocation()) {
...
//        WarrantixIntercomMessenger messenger = WarrantixIntercomMessenger.getInstance();
...
//                        startActivity(intent, true);
                    ...
                    case R.id.menu_warranty:
                        intent = new Intent(MainActivity.this, WalletBrandProducts.class);
                        intent.putExtra("for", "TransferWarranty");
                        startActivityForResult(intent, Constant.backCode, true);
                        return true;
...
//                Toast.makeText(WarrantixApplication.getAppContext(), "ToMarket = " + brandName, Toast.LENGTH_SHORT).show();
                ...
                if ((toScreen.equalsIgnoreCase("brand") == true) || (toScreen.equalsIgnoreCase("retailer") == true)) {
                    changeTab(5);
                } else if (toScreen.equalsIgnoreCase("settings")) {
                    Log.e("TEST --", "TEST --");
                    marketFragment.getBrandDataAndSetAdapter();
                } else {
                    changeTab(6);
...
//                                PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP);
            ...
            if (resultCode == RESULT_OK) {
                Log.d(TAG, "Plugin app installation is succeeded");
            } else {
                Log.d(TAG, "Plugin app installation is failed");
            }
        }
//        else if (requestCode == Constant.brandUpdated) {
//
...
//            Toast.makeText(MainActivity.this, "Customer Registration is called", Toast.LENGTH_SHORT).show();
    ...
    public void requestMarshMallowPermission(int skip) {


        DebugLog.e("requestMarshMallowPermission()--" + skip);
        // request camera permission
        if (skip < 1 && !marshMallowPermission.checkPermissionForCamera()) {
            marshMallowPermission.requestPermissionsForCameraNew();
        }
        // request external storage permission
        else if (skip < 2 && !marshMallowPermission.checkPermissionForExternalStorage()) {
            marshMallowPermission.requestPermissionsForExternalStorageNew();
        }
        // request fine location permission
        else if (skip < 3 && !marshMallowPermission.checkPermissionForFineLocation()) {
            marshMallowPermission.requestPermissionsForFineLocationNew();
        }
        // request phone call permission
        else if (skip < 4 && !marshMallowPermission.checkPermissionForDial()) {
            marshMallowPermission.requestPermissionsForDialNew();
        }
        // request send sms permission
        else if (skip < 5 && !marshMallowPermission.checkPermissionForSendSMS()) {
            marshMallowPermission.requestPermissionsForSendSmsNew();
        }
    }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        switch (requestCode) {
            ...
            case MarshMallowPermission.CAMERA_PERMISSION_REQUEST_CODE:
                requestMarshMallowPermission(1);
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    DebugLog.e("onRequestPermissionsResult: " + "Camera Permission Granted ..");
                } else {
                    DebugLog.e("onRequestPermissionsResult: " + "Camera Permission Denied ..");
                }
                break;

            case MarshMallowPermission.EXTERNAL_STORAGE_PERMISSION_REQUEST_CODE:
                requestMarshMallowPermission(2);
                break;

            case MarshMallowPermission.GPS_FINE_LOCATION_PERMISSION_REQUEST_CODE:
                requestMarshMallowPermission(3);
                break;

            case MarshMallowPermission.PHONE_CALL_PERMISSION_REQUEST_CODE:
                requestMarshMallowPermission(4);
                break;

            case MarshMallowPermission.SEND_SMS_PERMISSION_REQUEST_CODE:
                break;
