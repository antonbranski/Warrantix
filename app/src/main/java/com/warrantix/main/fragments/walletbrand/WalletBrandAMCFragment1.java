...
public class WalletBrandAMCFragment1 extends BaseFragment implements View.OnClickListener {
    ...
    public void InitializeView() {
        ...
        insurance_type.setTokenListView(amcTypeView, insuranceAdapter, amcTypeBTN);
        ...
        vendor.setTokenListView(serviceProviderView, providerAdapter,serviceProviderBTN);

        String deals[] = {"Deal 1", "Deal 2", "Deal 3"};
        TokenListAdapter dealAdapter = new TokenListAdapter((Activity) mContext, deals);
        deal.setTokenListView(dealView, dealAdapter, dealBTN);
        ...
        duration.setTokenListView(durationView, durationAdapter, durationBTN);

        final ImageView selectView = (ImageView) mView.findViewById(R.id.accept_select);
        selectView.setBackgroundResource(!((WalletBrandAMC)getActivity()).isAcceptedSelect ? R.drawable.square : R.drawable.square_tick);


        selectView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((WalletBrandAMC)getActivity()).isAcceptedSelect = !((WalletBrandAMC)getActivity()).isAcceptedSelect;
                selectView.setBackgroundResource(!((WalletBrandAMC)getActivity()).isAcceptedSelect ? R.drawable.square : R.drawable.square_tick);
            }
        });
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.nextBTN) {
            if (((WalletBrandAMC)getActivity()).isAcceptedSelect == false) {
